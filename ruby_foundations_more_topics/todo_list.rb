...
class TodoList
  ...
  def each
    # modify each so that it returns the original object
    todos.each do |todo|
      yield(todo)
    end
    self
  ...
  def select
    # modify select so that it returns a TodoList object
  ...
  def find_by_title(title)
    # returns first Todo object that matchs the argument
    todos.select { |todo| title == todo.title }.first
  end
...
puts list.select {|todo| todo.done?}.inspect
